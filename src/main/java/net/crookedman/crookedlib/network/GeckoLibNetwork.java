package net.crookedman.crookedlib.network;
...
import net.crookedman.crookedlib.network.packet.*;
...
public final class GeckoLibNetwork {
	...
	private static final SimpleChannel PACKET_CHANNEL = NetworkRegistry.newSimpleChannel(new ResourceLocation(CrookedLib.MOD_ID, "main"), () -> VER, VER::equals, VER::equals);
	 ...
	 * It is recommended that you don't call this directly, instead implementing and calling {@link net.crookedman.crookedlib.animatable.SingletonGeoAnimatable#registerSyncedAnimatable}
	 */
	synchronized public static void registerSyncedAnimatable(GeoAnimatable animatable) {
		...
		if (existing == null)
			CrookedLib.LOGGER.debug("Registered SyncedAnimatable for " + animatable.getClass().toString());
	}
	...
	public static GeoAnimatable getSyncedAnimatable(String className) {
		...
		if (animatable == null)
			CrookedLib.LOGGER.error("Attempting to retrieve unregistered synced animatable! (" + className + ")");
