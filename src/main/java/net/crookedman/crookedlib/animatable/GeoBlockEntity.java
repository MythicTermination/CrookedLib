package net.crookedman.crookedlib.animatable;
...
import net.crookedman.crookedlib.network.GeckoLibNetwork;
import net.crookedman.crookedlib.network.SerializableDataTicket;
import net.crookedman.crookedlib.network.packet.BlockEntityAnimDataSyncPacket;
import net.crookedman.crookedlib.network.packet.BlockEntityAnimTriggerPacket;
...
public interface GeoBlockEntity extends GeoAnimatable {
	...
	default <D> void setAnimData(SerializableDataTicket<D> dataTicket, D data) {
		...
		if (level == null) {
			CrookedLib.LOGGER.error("Attempting to set animation data for BlockEntity too early! Must wait until after the BlockEntity has been set in the world. (" + blockEntity.getClass().toString() + ")");
	 ...
	 * @param animName The name of animation to trigger. This needs to have been registered with the controller via {@link net.crookedman.crookedlib.core.animation.AnimationController#triggerableAnim AnimationController.triggerableAnim}
	 */
	default void triggerAnim(@Nullable String controllerName, String animName) {
		...
		if (level == null) {
			CrookedLib.LOGGER.error("Attempting to trigger an animation for a BlockEntity too early! Must wait until after the BlockEntity has been set in the world. (" + blockEntity.getClass().toString() + ")");
