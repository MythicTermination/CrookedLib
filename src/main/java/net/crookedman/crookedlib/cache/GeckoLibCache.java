package net.crookedman.crookedlib.cache;
...
import net.crookedman.crookedlib.loading.FileLoader;
import net.crookedman.crookedlib.loading.json.FormatVersion;
import net.crookedman.crookedlib.loading.json.raw.Model;
import net.crookedman.crookedlib.loading.object.BakedAnimations;
import net.crookedman.crookedlib.loading.object.BakedModelFactory;
import net.crookedman.crookedlib.loading.object.GeometryTree;
...
/**
 * Cache class for holding loaded {@link net.crookedman.crookedlib.core.animation.Animation Animations}
...
public final class GeckoLibCache {
	...
	public static Map<ResourceLocation, BakedAnimations> getBakedAnimations() {
		if (!CrookedLib.hasInitialized)
			throw new RuntimeException("CrookedLib was never initialized! Please read the documentation!");
	...
	public static void registerReloadListener() {
		...
		if (mc == null) {
			if (!ModLoader.isDataGenRunning())
				CrookedLib.LOGGER.warn("Minecraft.getInstance() was null, could not register reload listeners");

			return;
		}

		if (!(mc.getResourceManager() instanceof ReloadableResourceManager resourceManager))
			throw new RuntimeException("CrookedLib was initialized too early!");
	...
	private static CompletableFuture<Void> loadModels(Executor backgroundExecutor, ResourceManager resourceManager, BiConsumer<ResourceLocation, BakedGeoModel> elementConsumer) {
		return loadResources(backgroundExecutor, resourceManager, "geo", resource -> {
			...
			if (model.formatVersion() != FormatVersion.V_1_12_0)
				throw new CrookedLibException(resource, "Unsupported geometry json version. Supported versions: 1.12.0");
